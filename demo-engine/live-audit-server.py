...
"""
            ...
            try:
                ...
                # Persist the independently verified post-recovery ledger state.
                # This becomes the authoritative state restored after refresh.
                live_audit_path = OUTPUT / "live-audit.json"
                live_audit_path.write_text(
...
(async()=>{
  ...
  // ---------------------------------------------------------
  // DISCOVER EXACT SYNTHETIC RECOVERY JOURNAL
  // ---------------------------------------------------------
  ...
  const submitted=(list.data||[]).filter(j=>
    Number(j.docstatus)===1 &&
    String(j.user_remark||"").includes(marker)
  );
  ...
  if(submitted.length!==1){
    throw new Error(
      "SAFETY STOP: expected exactly one submitted "+
      "synthetic recovery journal; found "+
      submitted.length
    );
  }

  const name=submitted[0].name;
  ...
  const ap=rows.find(r=>
    r.account==="Creditors - GVD" &&
    r.party_type==="Supplier" &&
    r.party==="Summit Traders Ltd." &&
    Number(
      r.debit_in_account_currency||0
    )===12240
  );

  const rebate=rows.find(r=>
    r.account==="Supplier Rebates - GVD" &&
    Number(
      r.credit_in_account_currency||0
    )===12240
  );
  ...
  if(
    ...
    throw new Error(
      "SAFETY STOP: journal economics do not "+
      "match approved synthetic recovery."
    );
  }

  // ---------------------------------------------------------
  // CANCEL USING THE FULL DOCUMENT
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // INDEPENDENTLY RE-READ DOCUMENT
  // ---------------------------------------------------------
